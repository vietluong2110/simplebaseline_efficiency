...
for pred_len in 192 336 96 720; do
  for e_layers in 1 2; do
    for wv in  'bior3.1'; do
      for m in 1 3; do
        for lr in 0.0005 0.001 0.003 0.006; do
                ...
                for d_model in 256; do
                  for d_ff in 512; do
                    python -u run_l1s.py \
                      ...
                      --use_norm $use_norm \
                      --wv $wv \
                      --m $m >./logs_0927/ETTh2/'pl'$pred_len'_el'$e_layers'_wv'$wv'_m'$m'_lr'$lr'_bs'$batch_size'_seed'$fix_seed'_dm'$d_model'_dff'$d_ff.log
                  done
                done
              done
            done
