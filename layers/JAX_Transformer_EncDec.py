...
class JAX_EncoderLayer(nnx.Module):
    def __init__(self, attention, d_model, rngs: nnx.Rngs, d_ff=None, dropout=0.1, activation="relu", dec_in=866):
        ...
        self.conv1 = nnx.Conv(in_features=d_model, out_features=d_ff, kernel_size=(1, ), precision = jax.lax.Precision('highest'),
                               kernel_init = kernel_init, bias_init = bias_init, rngs = rngs)
        self.conv2 = nnx.Conv(in_features=d_ff, out_features=d_model, kernel_size=(1, ), precision = jax.lax.Precision('highest'), 
                                kernel_init = kernel_init, bias_init = bias_init, rngs = rngs)
