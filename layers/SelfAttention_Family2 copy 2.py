...
def optimized_swt_decomposition(x: Tensor, h0: Tensor, h1: Tensor, padding_cache: list, dilation_factors: Tensor, channels: int) -> Tensor:
    coeffs = []
    approx_coeffs = x
    
    for level, (pad_left, pad_right) in enumerate(padding_cache):
        dilation = dilation_factors[level].item()
        
        # Single padding operation
        approx_coeffs_pad = F.pad(approx_coeffs, (pad_left, pad_right), "circular")
        
        # Parallel convolution operations with grouped convolutions
        detail_coeff = F.conv1d(approx_coeffs_pad, h1, dilation=dilation, groups=channels)
        approx_coeffs = F.conv1d(approx_coeffs_pad, h0, dilation=dilation, groups=channels)
...
# @torch.compile(mode='max-autotune')
def optimized_swt_reconstruction(coeffs: Tensor, h0: Tensor, h1: Tensor, kernel_size: int, m: int, channels: int) -> Tensor:
    batch_size, channels, levels, seq_len = coeffs.shape
    dilation = 2 ** (m - 1)
    approx_coeff = coeffs[:,:,0,:]
    detail_coeffs = coeffs[:,:,1:,:]
    ...
    for i in range(m):
        ...
        # Optimized convolution operations
        approx_coeff = (F.conv1d(approx_coeff_pad, h0, groups=channels, dilation=dilation) + 
                       F.conv1d(detail_coeff_pad, h1, groups=channels, dilation=dilation)) / 2
...
class WaveletEmbedding(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        if self.swt:
            return optimized_swt_decomposition(x, self.h0, self.h1, self.padding_cache, 
                                            self.dilation_factors, self.d_channel)
        else:
            return optimized_swt_reconstruction(x, self.h0, self.h1, self.kernel_size, 
                                             self.m, self.d_channel)
...
class GeomAttentionLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        ...
        out = self.out_projection(out.permute(0, 3, 2, 1))#.mean(-2)
